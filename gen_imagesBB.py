...
# Additionally, if a bounding‐box mask is provided, the script will compute
# the bounding box from the mask (using a simple threshold) and draw it on
# the generated image.
# ----------------------------------------------------------------------------
...
# ----------------------------------------------------------------------------
def get_bbox_from_mask(mask: np.ndarray, threshold: float = 0.5) -> Optional[Tuple[int, int, int, int]]:
    """
    Given a 2D mask (numpy array with values in [0,1]), returns a bounding
    box (x0, y0, x1, y1) that tightly contains all pixels where mask > threshold.
    If no pixels exceed the threshold, returns None.
    """
    indices = np.where(mask > threshold)
    if indices[0].size == 0 or indices[1].size == 0:
        return None
    y0, y1 = int(indices[0].min()), int(indices[0].max())
    x0, x1 = int(indices[1].min()), int(indices[1].max())
    return (x0, y0, x1, y1)
...
@click.option('--bbox-mask', 'bbox_mask_path', type=str, default=None,
              help='Path to a bounding box mask PNG file or directory (grayscale). Must match the generator resolution.')
...
def generate_images(
    ...
    Optionally, provide a bounding box mask (or a directory containing mask PNGs).
    If a mask is provided, a bounding box will be drawn on the generated image.
    ...
    for seed_idx, seed in enumerate(seeds):
        ...
        # If a bounding box mask was provided, compute the bounding box and draw it.
        if current_bb_mask is not None:
            # Get the mask as a numpy array and squeeze the batch and channel dimensions.
            mask_np = current_bb_mask.squeeze().cpu().numpy()  # shape: [H, W]
            bbox = get_bbox_from_mask(mask_np, threshold=0.5)
            if bbox is not None:
                draw = ImageDraw.Draw(pil_img)
                # Draw a red rectangle with a width of 3 pixels.
                draw.rectangle(bbox, outline=(255, 0, 0), width=3)
                print(f'Drew bounding box: {bbox} on seed {seed} image.')
