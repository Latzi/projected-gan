# networks_fastgan.py
# -----------------------------------------------------------------------------
# Minimal modifications to accept bb_mask in the generator.
# -----------------------------------------------------------------------------
import torch
...
class FastganSynthesisCond(nn.Module):
    ...
    def forward(self, input, c, update_emas=False):
        ...
        """
        # Convert c from one-hot to ID, embed, etc.
...
class Generator(nn.Module):
    def __init__(
        ...
        self.bb_in_channels = 1    # <--- If you have multiple channels, set accordingly
        self.bb_project = nn.Linear(
            self.bb_in_channels * self.img_resolution * self.img_resolution,
            z_dim
        )

        # Dummy mapping to keep consistent with StyleGAN2 interface.
        self.mapping = DummyMapping()

        # Decide if conditional or not
    ...
    def forward(self, z, c, bb_mask=None, **kwargs):
        """
        z: [batch, z_dim]
        c: [batch, c_dim] or dummy
        bb_mask: [batch, M, H, W], e.g. M=1 for single‐channel bounding box
        """
        # If bounding‐box mask is given, flatten + embed => add to z.
        if bb_mask is not None:
            B, M, H, W = bb_mask.shape
            # We'll assume M == self.bb_in_channels
            # Flatten to [B, M*H*W]
            mask_flat = bb_mask.view(B, -1).float()  # shape [B, M*H*W]
            # Project to z_dim => shape [B, z_dim]
            mask_embed = self.bb_project(mask_flat)
            # Add or concat. We'll do add here:
            z = z + mask_embed

        # Normal forward pass
        w = self.mapping(z, c)           # => [B,1,z_dim]
